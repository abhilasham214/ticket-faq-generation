...
5. **Frontend**: on load, `Home` fetches `GET /api/faqs/latest` so there's always something to show; `FaqGenerator` drives generating something new - pick a CSV (or click "Try sample data"), click "Generate FAQs", get clusters back in one round trip. Themes render as tabs (`components/ui/tabs.tsx`, one `TabsTab`/`TabsPanel` per cluster); `ClusterCard` fills each panel with that theme's keywords, FAQ, numbered resolution steps, and escalation guidance, with the full list of source tickets (id, title, resolution) behind a "Show source tickets" disclosure. Each ticket has a "Discuss" toggle that opens `TicketChat`, a small inline Q&A scoped to that one ticket.
